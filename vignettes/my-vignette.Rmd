...
Test if esimates from each cluster give substantially different results (e.g. durban wu houseman test)

```{r}
Q <- 0
for(i in 1:max(dat[ , ncol(dat)]))
{
                bji <- res_clust[[i]]$betaWj
                Q <- Q + sum(w * (bji - res_clust[[i]]$coef)^2)
}
```
